...
// parameters
Std.atoi(me.arg(0)) => int songLength;
...
// countdown
0 => int countdown;
while (countdown < COUNTDOWN_BEATS) {
    countdown++;
    T => now;
}
...
while( Math.round(total) < songLength )
{
    if (s.gain() < GAIN_MAX) {
    ...
    if (total + t * PULSE_FACTOR > songLength){
        Math.max(0, (songLength - (e.attackTime() - e.decayTime())/1::ms - total))::ms => now;
        songLength => total;
    } else {
        Math.max(0, step)::ms => now;
        total + t * PULSE_FACTOR => total;
    }
}
